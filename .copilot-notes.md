...
### 1. Turnstile 人机验证
**架构**：Turnstile 模块支持多组件（`tokens`/`widgets` 为 Map，按 containerId 索引）。
**关键**：
- 不要在 `display:none` 容器内渲染，要在容器可见后再 `Turnstile.render()`
- `isVerified(containerId)` 指定容器检查；无参数则检查任意已验证
- `remove(containerId)` 清理弹窗内的组件（如反馈弹窗关闭时）
- 所有提交表单都需要 Turnstile：祝福墙、排行榜上榜、反馈弹窗
...
- Anon Key: `eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9...`（统一在 common.js 的 `MyLuck.getSupabase()`）
- Turnstile Site Key: `0x4AAAAAACZ5cJF8duhs4a2v`
- ⚠️ Supabase 客户端已合并为共享单例 `MyLuck.getSupabase()`，不要在其他文件重复创建
